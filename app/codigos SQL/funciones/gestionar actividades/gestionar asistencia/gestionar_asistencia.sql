...
begin
	...
	case opcion
		when 'registrar' then
			...
			(eve_id, asis_des, asis_estado, creado_por_usuario, creacion_fecha, modificado_por_usuario, modif_fecha)
			VALUES(eveid, v_asisdes, true, null, fecha_actual, null, null) returning asis_id into v_asisid;			
			raise notice 'Se insertó correctamente la cabecera = %',v_asisid;
			--Ahora cargar los arrays
			for i in 1..size_personas
			loop
				--Colectar de a uno				
		...
		when 'modificar' then
			...
			SET eve_id = eveid, asis_des = v_asisdes, 
