...
@jusa.route('/formulario_ausencia/<int:malla_id>/<int:cur_id>/<int:asi_id>/<int:num_id>/<turno>/<int:per_id>')
def formularioAusencia(malla_id, cur_id, asi_id, num_id, turno, per_id):
    form = Formulario()
    form.malla_id.data = malla_id
    form.cur_id.data = cur_id
    form.asi_id.data = asi_id
...
def guardar():
    ...
    if form.validate_on_submit():
        jusa_dao = JustificativoAlumno_dao()
        res = jusa_dao.guardar(form.alu_id.data, form.malla_id.data, form.cur_id.data, form.asi_id.data, form.num_id.data, form.idmaestro.data, form.turno.data, form.descripcion.data.upper(), int(session['usu_id']))
        if res > 0:
